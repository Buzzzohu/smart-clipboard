...
**[下载最新版 APK](https://github.com/Buzzzohu/smart-clipboard/releases/latest/download/Smart-Clipboard-0.10.3-custom-apps.apk)** · [查看版本发布页](https://github.com/Buzzzohu/smart-clipboard/releases)

> 当前版本：0.10.3。仓库目前为私有仓库，下载 Release 需要登录有访问权限的 GitHub 账号。

本次更新：应用联想支持从手机应用列表搜索并自行添加，提供独立开关与移除入口；长按卡片多选后可确认批量删除。侧边栏标题与底部分类管理固定，中间分类和筛选列表独立滚动。
...
- **自定义分类** — 在“侧边栏底部 → 分类管理”新增分类、改名、更换图片图标或删除分类。自带分类也可以编辑，仅“未分类”固定保留。删除分类时将内容移至“未分类”，不会删除内容。
- **搜索与筛选** — 按关键词查找内容；从屏幕左边缘右滑，打开分类和收藏筛选侧栏。
- **快捷管理** — 点击卡片复制内容；卡片左滑可进行编辑、删除、收藏等操作。长按进入多选，可全选当前列表、取消、统一修改分类或批量删除；删除前显示数量并要求确认。
...
- **滚动选择** — 在键盘上方显示浅色候选框，普通正文匹配最多 20 条；点击候选尝试填入当前输入框。
- **分类搜索** — 聊天框输入完整分类名（支持单字分类），优先显示该分类全部内容，上下滚动选择；没有同名分类才使用原来的正文联想。空分类不混入其他结果，关闭浮窗不保留分类筛选。
...
- **自行添加应用** — 在“设置 → 应用联想 → 添加应用”搜索手机上有启动入口的应用，添加后可独立开关或移除；原有预置应用开关保留。所有应用共用同一个 Smart Clipboard 无障碍服务，添加成功不代表输入框一定兼容。
- **预置应用** — QQ（`com.tencent.mobileqq`）、小黑盒（`com.max.xiaoheihe`）、bilibili（`tv.danmaku.bili`）、抖音标准版（`com.ss.android.ugc.aweme`）、JMComic2（`com.jiaohua_browser`）、JMComic3（`com.a7m3p9xv.t6qk2z8.app`）。B站和抖音优先支持评论、私信，不刻意排除其他普通输入框。
