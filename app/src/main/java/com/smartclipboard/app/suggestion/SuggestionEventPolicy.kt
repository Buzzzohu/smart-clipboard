...
/** Keeps the QQ overlay event decision independent of Android view objects. */
internal object SuggestionEventPolicy {
    ...
    fun decide(
        ...
        if (!activeQq) return Action.HIDE
        // The QQ message list is still in the same package, but has no active editor.
        ...
        if (!eventFromQq) return Action.IGNORE
